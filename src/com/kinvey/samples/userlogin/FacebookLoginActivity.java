...
import com.kinvey.samples.citywatch.CityWatchActivity;
import com.kinvey.samples.citywatch.KinveyService;
...
public class FacebookLoginActivity extends AccountAuthenticatorActivity {
	 ...
	 */
	private static final String FB_APP_ID = "289481461173872";//TODO
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mKinveyClient = KinveyService.getInstance(this).mKinveyClient;
	...
	private void loginFacebookKinveyUser(final ProgressDialog progressDialog, String accessToken) {
		...
		mKinveyClient.loginWithFacebookAccessToken(accessToken, new KinveyCallback<KinveyUser>() {
			...
			public void onSuccess(KinveyUser u) {
	            ...
	            FacebookLoginActivity.this.startActivity(new Intent(FacebookLoginActivity.this, CityWatchActivity.class));
	...
	private void finishConfirmCredentials(boolean result, String authKey, String password) {
		final Account account = new Account(authKey, KinveyService.ACCOUNT_TYPE);
		mAccountManager.setPassword(account, password);
		mAccountManager.setUserData(account, KinveyService.LOGIN_TYPE_KEY, PARAM_LOGIN_TYPE_FACEBOOK);
	...
	private void finishLogin(String authToken, String password) {
		final Account account = new Account(authToken, KinveyService.ACCOUNT_TYPE);
		if (mRequestNewAccount) {
			Bundle userData = new Bundle();
			userData.putString(KinveyService.LOGIN_TYPE_KEY, PARAM_LOGIN_TYPE_FACEBOOK);
		...
		intent.putExtra(AccountManager.KEY_ACCOUNT_TYPE, KinveyService.ACCOUNT_TYPE);
